...
import { generateCloudId, generateJiraCallContext } from '../../domain/entities/testing';
...
describe('JiraUserService', () => {
	describe('isAdmin', () => {
		it('should return false if user does not have ADMINISTER permission', async () => {
			...
			const result = await jiraService.isAdmin(atlassianUserId, jiraCallContext);
		...
		it('should throw a ForbiddenByJiraServiceError if checkPermissions returns a 403', async () => {
			...
			await expect(() =>
				jiraService.isAdmin(atlassianUserId, jiraCallContext),
